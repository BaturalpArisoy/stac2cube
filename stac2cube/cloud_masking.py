...
def get_cloud_layers(
    ...
    if dataarrays:
        ...
        if "cloud_mask" in cloud_layers:
            ...
            cloud_prob_uint8 = (cloud_only_stack.sel(band="cloud_prob") * 100).astype(np.uint8)
            # If cloud_mask is not present, create a single-band stack.
            cloud_stac_uint8 = cloud_prob_uint8.expand_dims(dim="band")
            cloud_stac_uint8 = cloud_stac_uint8.assign_coords(band=["cloud_prob"])
...
def mask_from_probability(cloud_probability, threshold=0.7, average_over=4, dilation_size=2):
    ...
    for t_val in thresholds:
        ...
        # Create the band label automatically based on the threshold value.
...
def get_cloud_layers(polygon, daterange, output, threshold=None, clip_raster=None, masking=None):
    ...
    # We use a default internal threshold (0.7) for computing cloud probability.
    average_over = 4
    dilation_size = 2
