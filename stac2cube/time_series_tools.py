...
def _add_top_label(im: Image.Image, txt: str, font_scale=0.03, font_min=14, font_max=48, bar_pad=None):
    """
...
def make_frame(
    ...
    if mode == "rgb":
        ...
        N = da.sel(time=t, band=nir_key).transpose("y", "x").compute().values.astype("float32", copy=False)
        R = da.sel(time=t, band=red_key).transpose("y", "x").compute().values.astype("float32", copy=False)
        ...
        G = da.sel(time=t, band=grn_key).transpose("y", "x").compute().values.astype("float32", copy=False)
        N = da.sel(time=t, band=nir_key).transpose("y", "x").compute().values.astype("float32", copy=False)
        ...
        raise ValueError("display_mode must be one of: 'rgb', 'false_color', 'ndvi', 'ndwi'")
    ...
    # label
    if label:
        im = _add_top_label(im, _format_date_ddmmyyyy(t),
                            font_scale=font_scale, font_min=font_min, font_max=font_max, bar_pad=bar_pad)
...
def save_timeseries_gif(
    ...
    frames = [make_frame(da, t, display_mode=display_mode, max_width=max_width, label=label) for t in times]
